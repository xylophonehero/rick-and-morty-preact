...
// src/utils/handlePromise.ts
var handlePromise = (promise) => promise.then((data) => [data, null]).catch((error) => Promise.resolve([null, error]));

// src/functions/getQuote.ts
var import_node_fetch = __toModule(require_lib());
var handler = async (event, context) => {
  const [data, err] = await handlePromise((0, import_node_fetch.default)("http://loremricksum.com/api/?paragraphs=1&quotes=1").then((res) => res.json()));
  if (err) {
    console.error(err);
    return {
      statusCode: 500,
